  ...
  return (
    <div style={{ background: 'white' }}>
      ...
      <div style={{ padding: '56px 48px 0' }}>
        <div style={{ borderRadius: 28, overflow: 'hidden', display: 'flex', height: 380, cursor: 'pointer' }}>
          <div style={{ flex: 1, background: `url(${getImageUrl(featured?.image, '/images/hero-port.jpg')}) center/cover no-repeat`, position: 'relative' }}>
          ...
          <div style={{ flex: 1, background: '#033059', padding: '48px 56px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            ...
            <button style={{ height: 48, padding: '0 28px', borderRadius: 48, border: '1px solid rgba(255,255,255,0.4)', background: 'transparent', fontFamily: 'Montserrat, sans-serif', fontWeight: 500, fontSize: 14, color: 'white', cursor: 'pointer', alignSelf: 'flex-start' }}>
              Lire l'article →
            </button>
          </div>
        </div>
          ...
          {posts.slice(0, 6).map((art, i) => (
            <div key={art._id}
              ...
              <div style={{ height: 220, position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '10px 14px', display: 'flex', gap: 8 }}>
                  {(art.tags || []).map(tag => (
